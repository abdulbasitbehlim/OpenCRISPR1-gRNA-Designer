...
External tools compared: CHOPCHOP.
...
**Build status:** external web/CLI execution was not reproducibly available in the artifact environment, therefore v1.3.1 does not invent a CHOPCHOP concordance count. The protocol and machine-readable comparison utility are included so the exact run can be committed before publication.
